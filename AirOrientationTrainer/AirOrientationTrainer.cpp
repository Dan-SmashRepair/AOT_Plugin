...
void AirOrientationTrainer::onLoad()
{
	...
	cvarManager->registerCvar("AOT_difficulty", "10", "Difficulty", true, true, 5, true, 25);
...
void AirOrientationTrainer::Render(CanvasWrapper canvas)
{
	...
	// difficulty 10 means you have to be within 10% of pitch, yaw and roll. Lower = harder
	...
	int rot_accuracy = difficulty * 654;
	(tickFlipFlop == true) ? tickFlipFlop = false : tickFlipFlop = true; // only reset every 2nd tick to avoid edge cases
	if (abs(targetPitch - carRotation.Pitch) < rot_accuracy / 2 && tickFlipFlop) {
		if (abs(targetYaw - carRotation.Yaw) < rot_accuracy || abs(targetYaw) >= 32768 - rot_accuracy && abs(carRotation.Yaw) >= 32768 - rot_accuracy) {
			if (abs(targetRoll - carRotation.Roll) < rot_accuracy || abs(targetRoll) >= 32768 - rot_accuracy && abs(carRotation.Roll) >= 32768 - rot_accuracy) {
	...
	// calculating then visualizing target orientation
	...
	// tutorial text. Only shows until one of the first few levels is completed in under 5s
	if (bestTimes.at(0) > 600 && bestTimes.at(1) > 600 && bestTimes.at(2) > 600){
		canvas.SetColor(255, 255, 0, 255);
		canvas.SetPosition(Vector2F{ 150.0, 220.0 });
		canvas.DrawString("Align the front of your car with the yellow arrow, wheels toward orange.");
	}
	...
	canvas.SetPosition(Vector2F{ 1.0, 630.0 });
